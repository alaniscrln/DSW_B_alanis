...
<body>
    ...
    <?php
        if (isset($_REQUEST['dni']) && isset($_REQUEST['nombre']) && isset($_REQUEST['apellido'])) {
            ...
            $sql = "UPDATE agenda SET Nombre = $nombre, Apellidos = $apellido WHERE dni = $dni";

            if(mysqli_query($conn, $sql)){
                echo "<br><h1><center><br><br>". $lang["bien_actualizado"] . "</center></h1>";
            } else{
                echo "<br><h1 class='text-danger'><center>" . $lang["mal_actualizado"] . " ". $sql . mysqli_error($conn) . "</center></h1>";
            }
